module RequestDataHook
  ...
  def self.header_start_time(headers)
    if headers
      ...
      HEADERS_NAMES.each do |header_name|
        ...
        if match.nil?
          RailsNetuitiveLogger.log.error "queue time header value #{header} is not recognized"
          next
        end
        header_time = match.to_s.to_f / RailsConfigManager.queue_time_divisor
        RailsNetuitiveLogger.log.debug "queue header_time: #{header_time}"
        start_time = start_time.nil? || header_time < start_time ? header_time : start_time
        RailsNetuitiveLogger.log.debug "queue start_time: #{start_time}"
      end
  ...
  def netuitive_request_hook
    ...
    begin
      ...
      RailsNetuitiveLogger.log.debug "queue_time: #{queue_time}"
      RailsNetuitiveLogger.log.debug 'sending queue_time metrics'
      NetuitiveRubyAPI.add_sample('action_controller.request.queue_time', queue_time)
      return unless netuitive_controller_name
      RailsNetuitiveLogger.log.debug "sending queue_time metrics with netuitive_controller_name #{netuitive_controller_name}"
      ...
      RailsNetuitiveLogger.log.debug "sending queue_time metrics with netuitive_action_name #{netuitive_action_name}"
      ...
      RailsNetuitiveLogger.log.error "exception during request tracking: message:#{e.message} backtrace:#{e.backtrace}"
    end
  end
end
